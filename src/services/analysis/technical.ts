...
import type { IndicatorSnapshot, TechnicalAnalysisResult, TimeframeAnalysis, Trend } from '@/domain/analysis';
...
const detectTrend = (indicators: IndicatorSnapshot): Trend => {
  ...
  if (indicators.ema20 > indicators.ema50 && (indicators.rsi ?? 50) >= 50) return 'BULLISH';
  if (indicators.ema20 < indicators.ema50 && (indicators.rsi ?? 50) <= 50) return 'BEARISH';
  return 'RANGING';
};
...
export function analyzeCandles(candles: Candle[]): TechnicalAnalysisResult {
  ...
  const indicators: IndicatorSnapshot = { ema20, ema50, sma200: average(closes.slice(-200)), rsi: rsi14 };
  ...
  if (rsi14 !== undefined) evidence.push(`RSI is ${rsi14.toFixed(1)}`);
  ...
  const volatility = closes.length > 1 && average(closes) ? Math.abs((closes.at(-1)! - closes[0]) / average(closes)) : 0;
  return {
